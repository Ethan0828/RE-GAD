...
class BaseDetector(object):
    ...
    def eval(self, labels, probs, new_label):
        score = {}
        with torch.no_grad():
            ...
            # print('min probs: ', min(probs), 'max probs: ', max(probs))
            # print(self.data.graph.ndata['label_1'])
            aucs = []
            ...
            for i in [i*0.01 for i in range(101)]:
                new_probs = i * probs + (1-i) * new_label
                aucs.append(roc_auc_score(labels, new_probs))
            # combined_label,_ = uncertainty_based_weighting(probs, new_label)
            score['AUROC'] = max(aucs)
            # score['AUROC'] = roc_auc_score(labels, combined_label)
            score['AUROC_list']=aucs
            # score['AUROC_stat'] = [min_probs, max_probs, np.average(probs), min(new_label), max(new_label), np.average(new_label)]
...
class BaseGNNDetector(BaseDetector):
    ...
    def train(self):
        ...
        for e in range(self.train_config['epochs']):
            ...
            val_score = self.eval(val_labels, probs[self.val_graph.ndata['val_mask']], self.val_graph.ndata['label_1'][self.val_graph.ndata['val_mask']])
            if val_score[self.train_config['metric']] > self.best_score:
                ...
                torch.save(probs, 'result.pt')
                test_score = self.eval(test_labels, probs[self.test_mask], self.val_graph.ndata['label_1'][self.test_mask])
...
class NAGNNDetector(BaseDetector):
    ...
    def train(self):
        ...
        for e in range(self.train_config['epochs']):
            ...
            val_score = self.eval(val_labels, probs[self.val_mask], self.val_graph.ndata['label_1'][self.val_mask])
            if val_score[self.train_config['metric']] > self.best_score:
                self.patience_knt = 0
                self.best_score = val_score[self.train_config['metric']]
                test_score = self.eval(test_labels, probs[self.test_mask], self.val_graph.ndata['label_1'][self.test_mask])
...
class SVMDetector(BaseDetector):
    ...
    def train(self):
        ...
        val_score = self.eval(val_y, pred_val_y, self.val_graph.ndata['label_1'][self.val_mask])
        self.best_score = val_score[self.train_config['metric']]
        test_score = self.eval(test_y, pred_y, self.val_graph.ndata['label_1'][self.test_mask])
        return test_score
