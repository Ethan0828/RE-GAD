...
class Dataset:
    ...
    def split(self, semi_supervised=True, trial_id=0):
        if semi_supervised:
            trial_id += 10
        print(self.graph.ndata['train_mask'].shape)
        self.graph.ndata['train_mask'] = self.graph.ndata['train_mask'][:,trial_id]
        self.graph.ndata['val_mask'] = self.graph.ndata['val_mask'][:,trial_id]
        self.graph.ndata['test_mask'] = self.graph.ndata['test_mask'][:,trial_id]
